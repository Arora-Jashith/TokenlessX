...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const calculateTrustScore = (user: User): number => {
    ...
    return Math.min(100, Math.max(0, baseScore + reputationBonus + transactionBonus + trustPointsBonus + networkBonus));
  };
  ...
  const addUser = (name: string, email: string, password: string, isAdmin: boolean) => {
    ...
    const activity: TrustNetworkActivity = {
      id: `activity-${Date.now()}`,
      type: 'user_creation',
      description: `New user ${name} created by ${currentUser?.email}`,
      timestamp: new Date().toISOString(),
      userId: currentUser?.id || ''
    };
    setTrustNetworkActivities(prev => [activity, ...prev]);
  ...
  const adjustTrustPoints = (userId: string, points: number, reason: string) => {
    ...
    const updatedUser = {
      ...user,
      trustPoints: Math.max(0, user.trustPoints + points),
      trustScore: calculateTrustScore({
            ...user,
        trustPoints: user.trustPoints + points
      })
    };
    ...
    const activity: TrustNetworkActivity = {
      ...
      description: `Trust points adjusted for ${user.name} by ${currentUser?.email}: ${points} points (${reason})`,
      timestamp: new Date().toISOString(),
      userId: currentUser?.id || ''
    };
    
    setTrustNetworkActivities(prev => [activity, ...prev]);
    toast.success('Trust points adjusted successfully');
  };
